...
public class BeehiveDAOImpl implements BeehiveDAO {
	...
	public void addBeehive(Beehive bh) {
		...
		try {
			...
			/*
			 * Persist: save to db and has void return 
			 * Save: save to db and returns serializable identifier value (id).
			 */
			sess.persist(bh);
	...
	public Beehive selectBeehiveById(int id) {
		...
		try {
			 ...
			 * Get vs Load
			 * 
			 * get() - Eager loading - immediately go to the database and get all the
			 * information associated with the object.
			 * 
			 * load() - Lazy loading - will return a proxy object with many fields not
			 * having a value. Those values are only loaded when they are specifically
	...
	public void updateBeehive(Beehive change) {
		...
		try {
			...
			/*
			 * Update vs. Merge Hibernate does not allow 2 objects with the same id in the
			 * same session.
			 * 
			 * 'Update' on an object in the session with the same id will throw an
			 * exception, because it doesn't know which object should be changed.
			 * 
